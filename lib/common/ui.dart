...
class Ui {
  ...
  static Html applyHtml(String html, {TextStyle style, TextAlign textAlign, Alignment alignment = Alignment.centerLeft}) {
    // Sanitize the style to remove invalid font features
    final safeStyle = style?.copyWith(fontFeatures: style.fontFeatures?.where((f) {
      // Only keep font features with 4-character tags
      return f.feature.length == 4;
    }).toList()) ?? Get.textTheme.bodyText1;
      ...
      customRender: {
        "p": (RenderContext context, Widget child) {
          return Text(
            context.tree.element.text,
            textAlign: textAlign,
            style: safeStyle.merge(TextStyle(fontSize: 11)),
          );
        },
      },
      style: {
        "*": Style(
          ...
          fontSize: FontSize(safeStyle.fontSize ?? 16.0),
          display: Display.INLINE_BLOCK,
          fontWeight: safeStyle.fontWeight ?? FontWeight.w300,
          width: Get.width,
          fontFeatureSettings: null, // Disable font features to be safe
        ),
        "li": Style(
          textAlign: textAlign,
          lineHeight: LineHeight.normal,
          listStylePosition: ListStylePosition.OUTSIDE,
          fontSize: FontSize(safeStyle.fontSize ?? 14.0),
          display: Display.BLOCK,
        ),
        "h4,h5,h6": Style(
          textAlign: textAlign,
          fontSize: FontSize((safeStyle.fontSize ?? 14.0) + 2),
        ),
        "h1,h2,h3": Style(
          textAlign: textAlign,
          lineHeight: LineHeight.number(2),
          fontSize: FontSize((safeStyle.fontSize ?? 14.0) + 4),
        ),
        "br": Style(
          height: 0,
        ),
